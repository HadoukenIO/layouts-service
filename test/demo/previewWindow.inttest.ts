...
itParameterized(
    ...
    createWindowTest(async (context, testOptions: PreviewTestOptions) => {
        ...
        const previewWin: _Window = await fin.Window.wrap({name: 'preview-snap-valid', uuid: 'layouts-service'});
...
itParameterized(
    ...
    createWindowTest(async (context, testOptions: CreateWindowData) => {
        ...
        const previewWin: _Window = await fin.Window.wrap({name: 'preview-tab-valid', uuid: 'layouts-service'});
        const windowBounds = await Promise.all([getBounds(windows[0]), getBounds(windows[1])]);
