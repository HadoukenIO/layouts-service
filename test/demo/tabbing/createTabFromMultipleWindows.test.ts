...
test('Create tab group from 2 windows', async (assert) => {
  ...
  const serviceApplication: Application = await fin.Application.wrap(
      {uuid: 'layouts-service', name: 'layouts-service'});


  // Act
  const scriptToExecute =
      `createTabGroupsFromTabBlob(${JSON.stringify(tabBlobs)})`;
  await executeJavascriptOnService(scriptToExecute);

  // Tab group should have been created
  const serviceChildWindows: Window[] =
      await serviceApplication.getChildWindows();

  const uuidTestPattern = new RegExp(
      '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$',
      'i');

  const newTabGroupWindow: Window|undefined =
      serviceChildWindows.find((window: Window) => {
        return window.identity.uuid === 'layouts-service' &&
            uuidTestPattern.test(window.identity.name!);
      });
  ...
  assert.is(
      win2Bounds.top,
      (tabBlobs[0].groupInfo.dimensions.y +
       tabBlobs[0].groupInfo.dimensions.tabGroupHeight));
 ...
 */
async function createTabbingWindow(
    page: string, uuid: string, left: number): Promise<Application> {
