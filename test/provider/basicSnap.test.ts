...
test.beforeEach(async () => {
  ...
  win1 = await fin.Window.wrap(
      {uuid: app1.identity.uuid, name: app1.identity.uuid});
  win2 = await fin.Window.wrap(
      {uuid: app2.identity.uuid, name: app2.identity.uuid});
});
...
test.failing(
    'should allow reregistration of a previously used identity', async t => {
      ...
      const win1 =
          await fin.Window.wrap({uuid: 'test-app-0', name: 'test-app-0'});
      const win2 =
          await fin.Window.wrap({uuid: 'test-app-1', name: 'test-app-1'});
      const win2Bounds = await getBounds(win2);
