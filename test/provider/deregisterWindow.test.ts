...
test('delayed deregister, snap with registered', async t => {
  ...
  win2 = await createChildWindow({
    ...
    defaultWidth: 200,
    url:
        'http://localhost:1337/demo/frameless-window-delayed-deregistered.html',
    frame: false
  });
...
test('deregister snapped window', async t => {
  ...
  win2 = await createChildWindow({
    ...
    defaultWidth: 200,
    url:
        'http://localhost:1337/demo/frameless-window-triggered-deregistered.html',
    frame: false
  });
...
test('no preview when deregistered - dragging registered', async t => {
  ...
  const previewWin =
      await getWindow({name: 'previewWindow-', uuid: 'layouts-service'});
