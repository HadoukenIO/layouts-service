import { test, GenericTestContext, AnyContext } from 'ava';
...
* To add an additional window layouts to be tested, simply add new entries there. 
*/
Object.keys(arrangements).forEach(num => {
    ...
    Object.keys(arrangements[count]).forEach(name => {
        test(`${count} windows - ${name}`, async t => {
            ...
            // This will spawn the required number of windows in the correct positions/groups
