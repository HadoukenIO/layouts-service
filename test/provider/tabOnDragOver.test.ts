...
test.skip('Tabset on dragover - basic drop', async t => {
  ...
  await dragWindowToOtherWindow(
      win1, 'top-left', win2, 'top-left', {x: -20, y: -20});
...
test('Tabset on dragover - drop on torn-out dropped window', async t => {
  ...
  await dragWindowToOtherWindow(
      win3, 'top-left', win2, 'top-left', {x: -20, y: -20});
  await delay(500);
 ...
 */
async function assertTabbed(
    win1: Window, win2: Window,
    t: GenericTestContext<AnyContext>): Promise<void> {
...
async function assertNotTabbed(
    win: Window, t: GenericTestContext<AnyContext>): Promise<void> {
  // TODO: Determine if the window is tabbed on the service side.
