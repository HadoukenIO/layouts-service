...
import { dragCornerToCorner, dragSideToSide, Corner } from './utils/dragWindowTo';
...
async function initWindows(t: GenericTestContext<any>, num: number, side?: Side) {
    ...
    if (num === 4) {
        await dragCornerToCorner(windows[1], 'bottom-left', windows[0], 'bottom-right', { x: 2, y: -10 });
        await dragCornerToCorner(windows[2], 'top-right', windows[0], 'bottom-right', { x: -10, y: 2 });
        await dragCornerToCorner(windows[3], 'top-left', windows[0], 'bottom-right', { x: 10, y: 2 });
    }
}
