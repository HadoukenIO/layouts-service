...
export class WindowInitializer {
  ...
  constructor(
      arrangements: ArrangementsType, windowPositions?: WindowPosition[],
  ...
  public async initWindows(num: number, arrangementName?: string):
      Promise<Window[]> {
    const windows: Window[] = new Array<Window>(num);

    for (let i = 0; i < num; i++) {
      windows[i] = await createChildWindow(
          {...(this._windowPositions[i]), ...this._windowOptions});
    }

    if (arrangementName && this._arrangements.hasOwnProperty(num) &&
        this._arrangements[num].hasOwnProperty(arrangementName)) {
...
const defaultWindowPositions = [
  ...
  {defaultTop: 900, defaultLeft: 300}, {defaultTop: 900, defaultLeft: 600},
  {defaultTop: 300, defaultLeft: 900}, {defaultTop: 600, defaultLeft: 900},
