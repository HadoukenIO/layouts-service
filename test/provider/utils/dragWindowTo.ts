...
export const dragWindowToOtherWindow = async (
    draggedWindow: Win, draggedCorner: Corner, targetWindow: Win,
    targetCorner: Corner, offset?: Point) => {
  const draggedBounds = await getBounds(draggedWindow);
  const targetBounds = await getBounds(targetWindow);

  const draggedDimensions: Point = {
    x: draggedBounds.right - draggedBounds.left,
    y: draggedBounds.bottom - draggedBounds.top
  };
  const targetDimensions: Point = {
    x: targetBounds.right - targetBounds.left,
    y: targetBounds.bottom - targetBounds.top
  };
  ...
  await dragWindowTo(
      draggedWindow, targetBounds.left + finalAdjustment.x,
      targetBounds.top + finalAdjustment.y);
};

export const dragSideToSide = async (
    draggedWindow: Win, draggedSide: Side, targetWindow: Win, targetSide: Side,
    offset?: Point) => {
  ...
  await dragWindowToOtherWindow(
      draggedWindow, draggedCorner, targetWindow, targetCorner,
